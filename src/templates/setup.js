export function setupTemplate() {
  ...
  <section class="sec">
    <div class="flex items-center mb-6">
      ...
      <h2 class="text-2xl">Your situation</h2>
    </div>

    <div class="field-group">
      <label class="field-label">Retirement savings</label>
      <p class="field-hint">Total nest egg, today's dollars.</p>
      <div class="iw">
        <span class="p">$</span>
    ...
    <div class="field-group">
      <label class="field-label">How long should it last?</label>
      <div class="flex items-center gap-4 mt-2">
        <input type="range" min="5" max="60" step="1"
          x-model.number="years" class="flex-1"
          aria-label="Retirement length">
        <span class="text-2xl font-semibold tnum text-ink
          w-[72px] text-right" x-text="years + ' yr'"></span>
      </div>
    ...
    <div class="field-group">
      <label class="field-label">What should remain at the end?</label>
      <div class="flex gap-2.5 flex-wrap mt-2" role="radiogroup"
      ...
      <template x-if="goalPreset === 'custom'">
        <div class="iw mt-3 max-w-[140px]">
          <input type="number" class="has-suf" min="0" max="300"
            step="5" x-model.number="retainPct"
            aria-label="Retain percentage">
          <span class="s">%</span>
        </div>
      </template>
    </div>
...
function portfolioSection() {
  ...
  <section class="sec">
    <div class="flex items-center mb-6">
      ...
      <h2 class="text-2xl">Your portfolio</h2>
    </div>

    <div class="flex gap-2.5 flex-wrap mb-4" role="radiogroup"
    ...
    <div class="flex gap-4 text-xs text-muted flex-wrap">
      <span class="flex items-center gap-1.5">
        <span class="w-2.5 h-2.5 rounded-sm bg-accent inline-block">
        </span>
        <span x-text="Math.round(sleeves.sp)+'% stocks'"></span>
      </span>
      <span class="flex items-center gap-1.5">
        <span class="w-2.5 h-2.5 rounded-sm bg-teal inline-block">
        </span>
        <span x-text="Math.round(sleeves.bd)+'% bonds'"></span>
      </span>
      <span class="flex items-center gap-1.5">
        <span class="w-2.5 h-2.5 rounded-sm bg-amber inline-block">
        </span>
        <span x-text="Math.round(sleeves.gd)+'% gold'"></span>
      </span>
      <span class="text-faint" x-text="allocNote"></span>
    </div>
    ...
    <template x-if="showCustomPortfolio">
      <div class="mt-6 pt-6 border-t border-line"
        ...
        <template x-if="portfolio.length === 0">
          <p class="text-sm text-muted py-3">
            No holdings yet &mdash; pick below.</p>
        </template>
        <div class="space-y-2 mb-5">
          <template x-for="p in portfolio" :key="p.id">
            <div class="flex items-center gap-3 bg-wash
              rounded-xl px-4 py-3">
              <div class="flex-1 min-w-0">
                <p class="text-sm font-semibold text-ink truncate"
                  x-text="assetName(p.id)"></p>
                <p class="text-xs text-faint truncate"
                  x-text="assetTicker(p.id)+' · '+assetYears(p.id)">
                </p>
              </div>
              <div class="iw w-[76px] shrink-0">
                <input type="number" class="has-suf text-sm !py-2"
                  min="0" max="100" step="1" :value="p.weight"
              ...
              <button type="button" @click="removeAsset(p.id)"
                ...
                class="w-8 h-8 rounded-lg text-faint
                  hover:text-rose hover:bg-rose-soft
                  transition-colors shrink-0 flex items-center
