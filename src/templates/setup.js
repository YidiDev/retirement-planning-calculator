export function setupTemplate() {
  ...
  <section class="mx-4 sm:mx-auto sm:max-w-2xl card mb-5">
    ...
    <h2 class="text-lg font-bold text-ink mb-5">Your situation</h2>
    ...
    <div class="flex items-center gap-3 mb-1">
      ...
      <span class="text-xl font-extrabold tnum w-20 text-right" x-text="years + ' yr'"></span>
    </div>
    <div class="flex justify-between text-xs text-faint mb-6"><span>5 yr</span><span>60 yr</span></div>

    <label class="block text-sm font-bold mb-2">What should be left at the end?</label>
    <div class="flex gap-2 flex-wrap mb-3" role="radiogroup" aria-label="Retirement goal">
...
function portfolioSection() {
  ...
  <section class="mx-4 sm:mx-auto sm:max-w-2xl card mb-5">
    ...
    <div class="flex gap-2 flex-wrap mb-5" role="radiogroup" aria-label="Portfolio strategy">
    ...
    <div class="mb-4">
      <p class="text-xs font-bold text-muted mb-2 uppercase tracking-wide">Simulation mix</p>
      ...
      <div>
        ...
        <div class="space-y-2 mb-4">
          <template x-for="p in portfolio" :key="p.id">
            <div class="flex items-center gap-2 bg-soft rounded-xl px-3 py-2.5 border border-border">
              ...
              <div class="input-wrap w-20 shrink-0">
                <input type="number" class="has-suffix text-sm !py-2" min="0" max="100" step="1"
              ...
              <button type="button" @click="removeAsset(p.id)"
                ...
                class="w-8 h-8 rounded-lg bg-surface border border-border text-muted font-bold
                  hover:bg-danger-light hover:text-danger hover:border-danger transition-colors
                  shrink-0 flex items-center justify-center">&times;</button>
            </div>
          </template>
        </div>
              ...
              <div class="flex flex-wrap gap-1.5">
                <template x-for="a in groupAssets(group)" :key="a.id">
                  <button type="button" @click="addAsset(a.id)"
                    ...
                    class="pill-btn" :class="isInPortfolio(a.id) && 'pill-btn-active'"
