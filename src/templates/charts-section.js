export function chartsSectionTemplate() {
  ...
  <template x-if="showResults && rows.length > 0">
    <section class="mx-4 sm:mx-auto sm:max-w-2xl card mb-5">
      <p class="text-xs font-bold tracking-widest uppercase text-primary mb-4">Charts</p>

      <div class="tab-bar mb-5" role="tablist" aria-label="Result charts">
        <template x-for="t in [{id:'income',label:'Income'},{id:'preserve',label:'Savings left'},{id:'spotlight',label:'Deep dive'}]" :key="t.id">
          <button type="button" role="tab" :aria-selected="resultTab === t.id"
            class="tab-btn" :class="resultTab === t.id && 'tab-btn-active'"
      ...
      <div x-show="resultTab === 'spotlight'" x-cloak>
        <p class="text-xs text-muted mb-3">
          Follow a single retirement month by month.
        </p>
        <div class="flex flex-wrap gap-2 items-center mb-3">
          ...
          <button type="button" @click="selectSpotWorst()" class="chip-btn">Worst</button>
          <button type="button" @click="selectSpotBest()" class="chip-btn">Best</button>
          <button type="button" @click="selectSpotTypical()" class="chip-btn">Typical</button>
        </div>
